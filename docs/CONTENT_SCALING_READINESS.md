...
- geometry-assembled layered paintings, monolithic paintings, or seeded procedural presentation;
...
- Stage images load only for the active run; scenery and reusable painted tile kits do not inflate unrelated stages.
...
New campaign maps, alternate layouts, multi-route missions, wave scripts, economy curves, objectives already represented by the objective union, procedural palettes, landmarks, layered painted scenery/tile placement, and stage metadata require data changes only.
...
- `sunken-way`: 12-wave layered-painted campaign map whose road and foundations are generated from authoritative geometry over scenery-only art.
